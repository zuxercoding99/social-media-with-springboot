

SET statement_timeout = 0;
SET lock_timeout = 0;
SET idle_in_transaction_session_timeout = 0;
SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;
SELECT pg_catalog.set_config('search_path', '', false);
SET check_function_bodies = false;
SET xmloption = content;
SET client_min_messages = warning;
SET row_security = off;

SET default_tablespace = '';

SET default_table_access_method = heap;

--
-- Name: comments; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.comments (
    id bigint NOT NULL,
    content text NOT NULL,
    created_at timestamp(6) with time zone NOT NULL,
    post_id uuid NOT NULL,
    user_id uuid NOT NULL
);


ALTER TABLE public.comments OWNER TO myuser;

--
-- Name: comments_seq; Type: SEQUENCE; Schema: public; Owner: myuser
--

CREATE SEQUENCE public.comments_seq
    START WITH 1
    INCREMENT BY 50
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;


ALTER SEQUENCE public.comments_seq OWNER TO myuser;

--
-- Name: files; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.files (
    id bigint NOT NULL,
    content_type character varying(255) NOT NULL,
    created_at timestamp(6) with time zone NOT NULL,
    file_key character varying(255) NOT NULL,
    original_name character varying(255) NOT NULL,
    size bigint NOT NULL,
    url character varying(255) NOT NULL,
    user_id uuid NOT NULL,
    post_id uuid NOT NULL
);


ALTER TABLE public.files OWNER TO myuser;

--
-- Name: files_id_seq; Type: SEQUENCE; Schema: public; Owner: myuser
--

ALTER TABLE public.files ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.files_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1
);


--
-- Name: friends; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.friends (
    id bigint NOT NULL,
    created_at timestamp(6) with time zone,
    status character varying(255) NOT NULL,
    updated_at timestamp(6) with time zone,
    receiver_id uuid NOT NULL,
    requester_id uuid NOT NULL,
    CONSTRAINT friends_status_check CHECK (((status)::text = ANY ((ARRAY['PENDING'::character varying, 'ACCEPTED'::character varying, 'REJECTED'::character varying, 'BLOCKED'::character varying])::text[])))
);


ALTER TABLE public.friends OWNER TO myuser;

--
-- Name: friends_seq; Type: SEQUENCE; Schema: public; Owner: myuser
--

CREATE SEQUENCE public.friends_seq
    START WITH 1
    INCREMENT BY 50
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;


ALTER SEQUENCE public.friends_seq OWNER TO myuser;

--
-- Name: messages; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.messages (
    id bigint NOT NULL,
    content character varying(2000) NOT NULL,
    sent_at timestamp(6) with time zone NOT NULL,
    friend_id bigint NOT NULL,
    sender_id uuid NOT NULL
);


ALTER TABLE public.messages OWNER TO myuser;

--
-- Name: messages_id_seq; Type: SEQUENCE; Schema: public; Owner: myuser
--

ALTER TABLE public.messages ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.messages_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1
);


--
-- Name: post_likes; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.post_likes (
    id bigint NOT NULL,
    created_at timestamp(6) with time zone NOT NULL,
    post_id uuid NOT NULL,
    user_id uuid NOT NULL
);


ALTER TABLE public.post_likes OWNER TO myuser;

--
-- Name: post_likes_seq; Type: SEQUENCE; Schema: public; Owner: myuser
--

CREATE SEQUENCE public.post_likes_seq
    START WITH 1
    INCREMENT BY 50
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;


ALTER SEQUENCE public.post_likes_seq OWNER TO myuser;

--
-- Name: posts; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.posts (
    id uuid NOT NULL,
    content text,
    created_at timestamp(6) with time zone NOT NULL,
    privacy character varying(255) NOT NULL,
    updated_at timestamp(6) with time zone,
    user_id uuid NOT NULL,
    CONSTRAINT posts_privacy_check CHECK (((privacy)::text = ANY ((ARRAY['PUBLIC'::character varying, 'PRIVATE'::character varying, 'FRIENDS'::character varying])::text[])))
);


ALTER TABLE public.posts OWNER TO myuser;

--
-- Name: refresh_tokens; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.refresh_tokens (
    id uuid NOT NULL,
    expires_at timestamp(6) with time zone NOT NULL,
    token character varying(255) NOT NULL,
    user_id uuid NOT NULL
);


ALTER TABLE public.refresh_tokens OWNER TO myuser;

--
-- Name: roles; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.roles (
    id bigint NOT NULL,
    name character varying(255) NOT NULL
);


ALTER TABLE public.roles OWNER TO myuser;

--
-- Name: roles_seq; Type: SEQUENCE; Schema: public; Owner: myuser
--

CREATE SEQUENCE public.roles_seq
    START WITH 1
    INCREMENT BY 50
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;


ALTER SEQUENCE public.roles_seq OWNER TO myuser;

--
-- Name: users; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.users (
    id uuid NOT NULL,
    avatar_key character varying(255) NOT NULL,
    banner_color character varying(7),
    bio character varying(101),
    birth_date date NOT NULL,
    created_at timestamp(6) with time zone,
    display_name character varying(30) NOT NULL,
    email character varying(254) NOT NULL,
    enabled boolean NOT NULL,
    password character varying(200) NOT NULL,
    theme_mode character varying(10) NOT NULL,
    updated_at timestamp(6) with time zone,
    username character varying(30) NOT NULL,
    CONSTRAINT users_theme_mode_check CHECK (((theme_mode)::text = ANY ((ARRAY['LIGHT'::character varying, 'DARK'::character varying])::text[])))
);


ALTER TABLE public.users OWNER TO myuser;

--
-- Name: users_roles; Type: TABLE; Schema: public; Owner: myuser
--

CREATE TABLE public.users_roles (
    user_id uuid NOT NULL,
    roles_id bigint NOT NULL
);


ALTER TABLE public.users_roles OWNER TO myuser;

--
-- Name: comments comments_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.comments
    ADD CONSTRAINT comments_pkey PRIMARY KEY (id);


--
-- Name: files files_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.files
    ADD CONSTRAINT files_pkey PRIMARY KEY (id);


--
-- Name: friends friends_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.friends
    ADD CONSTRAINT friends_pkey PRIMARY KEY (id);


--
-- Name: messages messages_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.messages
    ADD CONSTRAINT messages_pkey PRIMARY KEY (id);


--
-- Name: post_likes post_likes_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.post_likes
    ADD CONSTRAINT post_likes_pkey PRIMARY KEY (id);


--
-- Name: posts posts_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.posts
    ADD CONSTRAINT posts_pkey PRIMARY KEY (id);


--
-- Name: refresh_tokens refresh_tokens_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.refresh_tokens
    ADD CONSTRAINT refresh_tokens_pkey PRIMARY KEY (id);


--
-- Name: roles roles_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.roles
    ADD CONSTRAINT roles_pkey PRIMARY KEY (id);


--
-- Name: files uk32q0l95i1j2wll47v91hracbq; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.files
    ADD CONSTRAINT uk32q0l95i1j2wll47v91hracbq UNIQUE (file_key);


--
-- Name: post_likes uk5l2rj28vw5oj6f7ox746grokg; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.post_likes
    ADD CONSTRAINT uk5l2rj28vw5oj6f7ox746grokg UNIQUE (post_id, user_id);


--
-- Name: users uk6dotkott2kjsp8vw4d0m25fb7; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT uk6dotkott2kjsp8vw4d0m25fb7 UNIQUE (email);


--
-- Name: refresh_tokens uk7tdcd6ab5wsgoudnvj7xf1b7l; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.refresh_tokens
    ADD CONSTRAINT uk7tdcd6ab5wsgoudnvj7xf1b7l UNIQUE (user_id);


--
-- Name: files uk9a2mib6fwg7u751x82ie3pq3k; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.files
    ADD CONSTRAINT uk9a2mib6fwg7u751x82ie3pq3k UNIQUE (post_id);


--
-- Name: refresh_tokens ukghpmfn23vmxfu3spu3lfg4r2d; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.refresh_tokens
    ADD CONSTRAINT ukghpmfn23vmxfu3spu3lfg4r2d UNIQUE (token);


--
-- Name: friends ukkfpr9vjrin0b2d6k5of006l9c; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.friends
    ADD CONSTRAINT ukkfpr9vjrin0b2d6k5of006l9c UNIQUE (requester_id, receiver_id);


--
-- Name: roles ukofx66keruapi6vyqpv6f2or37; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.roles
    ADD CONSTRAINT ukofx66keruapi6vyqpv6f2or37 UNIQUE (name);


--
-- Name: users ukr43af9ap4edm43mmtq01oddj6; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT ukr43af9ap4edm43mmtq01oddj6 UNIQUE (username);


--
-- Name: users users_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.users
    ADD CONSTRAINT users_pkey PRIMARY KEY (id);


--
-- Name: users_roles users_roles_pkey; Type: CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.users_roles
    ADD CONSTRAINT users_roles_pkey PRIMARY KEY (user_id, roles_id);


--
-- Name: idx_files_created_at; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_files_created_at ON public.files USING btree (created_at);


--
-- Name: idx_files_user_id; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_files_user_id ON public.files USING btree (user_id);


--
-- Name: idx_friends_receiver; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_friends_receiver ON public.friends USING btree (receiver_id);


--
-- Name: idx_friends_requester; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_friends_requester ON public.friends USING btree (requester_id);


--
-- Name: idx_friends_status; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_friends_status ON public.friends USING btree (status);


--
-- Name: idx_messages_friend; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_messages_friend ON public.messages USING btree (friend_id);


--
-- Name: idx_messages_sent_at; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_messages_sent_at ON public.messages USING btree (sent_at);


--
-- Name: idx_post_likes_post; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_post_likes_post ON public.post_likes USING btree (post_id);


--
-- Name: idx_post_likes_user; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_post_likes_user ON public.post_likes USING btree (user_id);


--
-- Name: idx_users_created_at; Type: INDEX; Schema: public; Owner: myuser
--

CREATE INDEX idx_users_created_at ON public.users USING btree (created_at);


--
-- Name: refresh_tokens fk1lih5y2npsf8u5o3vhdb9y0os; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.refresh_tokens
    ADD CONSTRAINT fk1lih5y2npsf8u5o3vhdb9y0os FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;


--
-- Name: messages fk2j3y616xg1shmonebnal9eglp; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.messages
    ADD CONSTRAINT fk2j3y616xg1shmonebnal9eglp FOREIGN KEY (friend_id) REFERENCES public.friends(id);


--
-- Name: users_roles fk2o0jvgh89lemvvo17cbqvdxaa; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.users_roles
    ADD CONSTRAINT fk2o0jvgh89lemvvo17cbqvdxaa FOREIGN KEY (user_id) REFERENCES public.users(id);


--
-- Name: messages fk4ui4nnwntodh6wjvck53dbk9m; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.messages
    ADD CONSTRAINT fk4ui4nnwntodh6wjvck53dbk9m FOREIGN KEY (sender_id) REFERENCES public.users(id);


--
-- Name: friends fk5c5mwkpdeyhhbhulwujqhv4al; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.friends
    ADD CONSTRAINT fk5c5mwkpdeyhhbhulwujqhv4al FOREIGN KEY (receiver_id) REFERENCES public.users(id);


--
-- Name: posts fk5lidm6cqbc7u4xhqpxm898qme; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.posts
    ADD CONSTRAINT fk5lidm6cqbc7u4xhqpxm898qme FOREIGN KEY (user_id) REFERENCES public.users(id);


--
-- Name: friends fk5mwylmmw7ot92jog0004x3gn1; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.friends
    ADD CONSTRAINT fk5mwylmmw7ot92jog0004x3gn1 FOREIGN KEY (requester_id) REFERENCES public.users(id);


--
-- Name: comments fk8omq0tc18jd43bu5tjh6jvraq; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.comments
    ADD CONSTRAINT fk8omq0tc18jd43bu5tjh6jvraq FOREIGN KEY (user_id) REFERENCES public.users(id);


--
-- Name: post_likes fka5wxsgl4doibhbed9gm7ikie2; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.post_likes
    ADD CONSTRAINT fka5wxsgl4doibhbed9gm7ikie2 FOREIGN KEY (post_id) REFERENCES public.posts(id) ON DELETE CASCADE;


--
-- Name: users_roles fka62j07k5mhgifpp955h37ponj; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.users_roles
    ADD CONSTRAINT fka62j07k5mhgifpp955h37ponj FOREIGN KEY (roles_id) REFERENCES public.roles(id);


--
-- Name: files fkdgr5hx49828s5vhjo1s8q3wdp; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.files
    ADD CONSTRAINT fkdgr5hx49828s5vhjo1s8q3wdp FOREIGN KEY (user_id) REFERENCES public.users(id);


--
-- Name: comments fkh4c7lvsc298whoyd4w9ta25cr; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.comments
    ADD CONSTRAINT fkh4c7lvsc298whoyd4w9ta25cr FOREIGN KEY (post_id) REFERENCES public.posts(id);


--
-- Name: files fkkb7cbgxj34ff0yfuf7cqhhh1y; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.files
    ADD CONSTRAINT fkkb7cbgxj34ff0yfuf7cqhhh1y FOREIGN KEY (post_id) REFERENCES public.posts(id);


--
-- Name: post_likes fkkgau5n0nlewg6o9lr4yibqgxj; Type: FK CONSTRAINT; Schema: public; Owner: myuser
--

ALTER TABLE ONLY public.post_likes
    ADD CONSTRAINT fkkgau5n0nlewg6o9lr4yibqgxj FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;




